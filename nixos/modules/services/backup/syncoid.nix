...
let
  ...
  buildAllowCommand = action: permissions: dataset: lib.escapeShellArgs [
    "zfs"
    action
    cfg.user
  ...
  # source dataset.
  buildSourceAllowScript = permissions: dataset: (
  ...
  buildSourceUnallowScript = permissions: dataset: (
  ...
  buildTargetAllowScript = permissions: dataset: (
    "-+${pkgs.writeShellScript "zfs-allow-${dataset}" ''
  ...
  buildTargetUnallowScript = permissions: dataset: (
    "-+${pkgs.writeShellScript "zfs-unallow-${dataset}" ''
      ${buildAllowCommand "unallow" permissions dataset}
      # Remove the last part of the path
